...
public abstract class ObjectService<T extends TLAObject> {
	 ...
	 * <ol>
	 * <li>specified in <code>application.yml</code> under the
	 * <code>search.{type}</code> path</li>
	 * <li>defined in a {@link SearchProperties} subclass with a {@link ModelClass}
	 * annotation</li>
	 * </ol>
	 */
	...
	public Optional<ObjectDetails<T>> getDetails(String id) {
		...
		try {
			...
			return Optional.of(new ObjectDetails<T>((T) container.getObject(), container.getRelated()));
		} catch (Exception e) {
